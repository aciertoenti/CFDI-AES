...
import ConsolidarPublicoGeneralModal from "./ConsolidarPublicoGeneralModal";
...
export default function Emisores() {
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}><div><SectionTitle>Emisores</SectionTitle><SectionSub>Clic en una tarjeta para cambiar el emisor activo.</SectionSub></div><Btn type="button" variant="secondary" onClick={()=>setMostrandoFormulario(true)}>+ Agregar otro emisor</Btn></div><TwoCol>{emisores.map(emisor=>{const activo=emisor.rfc===emisorActivoRfc;return <Card key={emisor.rfc} style={{cursor:"pointer",border:`1px solid ${activo?C.accent:C.border}`}} onClick={()=>setEmisorActivoRfc(emisor.rfc)}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:8}}><div style={{minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{emisor.razon_social}</div><div style={{fontSize:12,color:C.textMuted,fontFamily:"monospace",marginTop:2}}>{emisor.rfc}</div></div><div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end",flexShrink:0}}>{activo&&<span style={{background:C.accentSoft,color:C.accentBorder,fontSize:11,fontWeight:600,padding:"3px 10px",borderRadius:20}}>✓ Seleccionado</span>}<span style={{background:emisor.estado==="Activo"?C.accentSoft:C.dangerSoft,color:emisor.estado==="Activo"?C.accentBorder:C.danger,fontSize:11,fontWeight:600,padding:"3px 10px",borderRadius:20}}>{emisor.estado}</span></div></div><div style={{fontSize:13,color:C.textSec,marginBottom:4}}>Régimen fiscal: <strong style={{color:C.text}}>{emisor.regimen_fiscal}</strong></div><div style={{fontSize:13,color:C.textSec,marginBottom:4}}>CP expedición: <strong style={{color:C.text}}>{emisor.codigo_postal}</strong></div><div style={{fontSize:13,color:C.textSec,marginBottom:4}}>Dado de alta por: <strong style={{color:C.text}}>{emisor.creado_por_rfc||"—"}</strong></div><div style={{fontSize:13,color:C.textSec,marginBottom:10}}>Modificado por: <strong style={{color:C.text}}>{emisor.modificado_por_rfc||"—"}</strong></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><Btn type="button" variant="secondary" style={{flex:"1 1 45%",fontSize:12,padding:"6px 10px"}} onClick={e=>{e.stopPropagation();setAccionActiva({tipo:"editar",emisor});}}>Editar</Btn><Btn type="button" variant="secondary" style={{flex:"1 1 45%",fontSize:12,padding:"6px 10px"}} onClick={e=>{e.stopPropagation();setAccionActiva({tipo:"csd",emisor});}}>Reemplazar certificado</Btn>{emisor.periodicidad_consolidacion&&<Btn type="button" variant="secondary" style={{flex:"1 1 45%",fontSize:12,padding:"6px 10px"}} onClick={e=>{e.stopPropagation();setAccionActiva({tipo:"consolidar",emisor});}}>Consolidar Público en General</Btn>}<Btn type="button" variant="secondary" disabled={togglingRfc===emisor.rfc} style={{flex:"1 1 45%",fontSize:12,padding:"6px 10px"}} onClick={e=>{e.stopPropagation();toggleEstado(emisor);}}>{togglingRfc===emisor.rfc?"…":(emisor.estado==="Activo"?"Inactivar":"Activar")}</Btn><Btn type="button" variant="secondary" style={{flex:"1 1 45%",fontSize:12,padding:"6px 10px",color:C.danger,borderColor:C.danger}} onClick={e=>{e.stopPropagation();setAccionActiva({tipo:"eliminar",emisor});}}>Eliminar</Btn></div></Card>;})}</TwoCol>
    ...
    {accionActiva?.tipo==="consolidar"&&<ConsolidarPublicoGeneralModal emisor={accionActiva.emisor} onCerrar={cerrarAccion} recargar={recargar}/>}
  </div>;
}
