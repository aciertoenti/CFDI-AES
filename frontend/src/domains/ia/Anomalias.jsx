...
import { useFacturas } from "../facturacion/hooks";
...
export default function Anomalias(){
  ...
  const sinFacturas=!loadingFacturas&&facturas.length===0;
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,gap:12,flexWrap:"wrap"}}>
        ...
        <Btn onClick={correr} disabled={loading||loadingFacturas||sinFacturas}>{loading?"Analizando…":"Analizar ahora →"}</Btn>
      </div>
      {loadingFacturas && <p style={{color:C.textMuted}}>Cargando tus facturas...</p>}
      {sinFacturas && <p style={{color:C.textMuted}}>No hay facturas para analizar todavía.</p>}
      {!ran&&!loading&&!error&&!sinFacturas&&<Card style={{textAlign:"center",padding:48}}><div style={{fontSize:36,marginBottom:10}}>🔍</div><div style={{fontSize:14,fontWeight:600,color:C.text}}>Sin análisis todavía</div></Card>}
